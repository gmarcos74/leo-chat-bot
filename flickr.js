...
function FlickrAddon() {

    function flickrSearchUserID() {
    ...
    function flickrConstructURL(photoInfo) {
    ...
    async function flickrPhotoInfo(photoInfo) {
        var photo_info = {};
    ...
    async function flickrGetRandomImages() {
        ...
        if (results && results.text) {
            ...
            if (jsonResults.photos && jsonResults.photos.photo && jsonResults.photos.photo.length) {
                ...
                for (var photoElementIndex in jsonResults.photos.photo) {
                    ...
                    photoElement.base_url = flickrConstructURL(photoElement);
                    photoElement.photo_info = await flickrPhotoInfo(photoElement);
    ...
    return {
        flickrGetRandomImages: flickrGetRandomImages
    }
}

module.exports = FlickrAddon();